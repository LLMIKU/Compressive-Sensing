...
void main()
{
	...
	//%% << "偏差矩阵和自身转置的乘积得到的矩阵：\n" << temp << endl;
	Timer("*求整体偏差矩阵", start, middle, finish, totaltime);
	...
	Timer("hadmard", start, middle, finish, totaltime);
	...
	//%% << "Y:" << endl << Y << endl;
	cout << "Y.row is " << Y.rows() << "Y.col is " << Y.cols() << endl;
	...
	for (i = 0; i < row; i++)
	{
		...
		for (j = 0; j < col; j++)
		{
			...
			{
				ptmp[j] = 0;
				printf("小于0情况：像素(%d,%d)处是%lf\n", i, j, recon_I(i, j));
			}	
			else if (recon_I(i, j) > 255.0)
			{
				ptmp[j] = 254;
				printf("大于255情况：像素(%d,%d)处是%lf\n", i, j, recon_I(i, j));
			}		
			else
				ptmp[j] = recon_I(i, j);*/
		}
